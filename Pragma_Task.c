...
int main(int argc, char *argv[]){
	...
	printf("\nNumber of threads: %d\n", x);
	// Assign 5 more buckets than threads
	buckets = x+5;
...
void bucket_sort(double *A,int num_elem,int buckets){
	...
	// Implement the task queue. One thread assigns a task (one for every bucket to be sorted) to all the other threads
	#pragma omp parallel
	{
		#pragma omp single private(i) nowait
		{
			for(i=0;i<buckets;i++){
				#pragma omp task firstprivate(i) shared(ptr_table,matrixes,matrixes_size)
				{
